...
allEntityIds ::
    WorkArea v name i o (Sugar.Payload (v EvalValues) name i o (t, [EntityId])) ->
    [(EntityId, IsHidden)]
...
validateHiddenEntityIds ::
    WorkArea v name i o (Sugar.Payload (v EvalValues) name i o (t, [EntityId])) -> Either String ()
...
validate ::
    (NFData (v EvalValues), NFData t, NFData name) =>
    WorkArea v name (T fa) (T fb)
    (Sugar.Payload (v EvalValues) name (T fa) (T fb) (t, [EntityId])) ->
    T ViewM
    (WorkArea v name (T fa) (T fb)
        (Sugar.Payload (v EvalValues) name (T fa) (T fb) (t, [EntityId])))
validate workArea
...
convertWorkArea ::
    ...
    (WorkArea (EvaluationScopes Name (T ViewM)) Name (T ViewM) (T ViewM)
        (Sugar.Payload
            (EvaluationScopes Name (T ViewM) EvalValues) Name (T ViewM) (T ViewM)
            (ParenInfo, [EntityId])))
convertWorkArea env =
