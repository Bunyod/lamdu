...
class Order v name i o t where
    order :: OrderT i (t # Annotated (Sugar.Payload (v Sugar.EvalValues) name i o a))

    default order ::
        ( MonadTransaction m i, HTraversable t
        , HNodesConstraint t (Order v name i o)
        ) =>
        OrderT i (t # Annotated (Sugar.Payload (v Sugar.EvalValues) name i o a))
...
orderParams ::
    MonadTransaction m i =>
    OrderT i [(Sugar.FuncParam val name, Sugar.ParamInfo name i o)]
orderParams xs =
    xs
...
orderNode ::
    ...
    OrderT i (Annotated (Sugar.Payload (v Sugar.EvalValues) name i o a) # f)
...
orderDef ::
    MonadTransaction m i =>
    OrderT i (Sugar.Definition v name i o (Sugar.Payload (v Sugar.EvalValues) name i o a))
orderDef def =
    def
