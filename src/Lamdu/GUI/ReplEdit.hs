...
extractEventMap ::
    ...
    env -> Sugar.Payload val name i (T m) a -> [MetaKey] -> EventMap (T m GuiState.Update)
...
replEventMap ::
    ...
    env -> ExportRepl m -> Sugar.Payload val name i (T m) a ->
...
make ::
    ...
    (Sugar.Payload (Sugar.EvaluationScopes Name (T m) Sugar.EvalValues) Name (T m) (T m) ExprGui.Payload) ->
